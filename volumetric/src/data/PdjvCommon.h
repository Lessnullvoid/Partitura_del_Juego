...
inline void assignRegion(int seed, PersistentPoint& p) {
    ...
    p.angular = ((std::abs(seed) * 47 % 1009) / 1008.f) * 6.28318530718f;
...
inline void anatomicalTarget(const PlayerObservation& player,
    ...
    if (p.jointA >= 0 && p.jointB >= 0 &&
        ...
        if (p.bodyRegionId == 1)
            thick = 0.040f;
        else if (p.bodyRegionId == 2)
            thick = 0.085f;
        else if (p.bodyRegionId == 5 || p.bodyRegionId == 6)
            thick = 0.036f;
        else
            thick = 0.02f;
        if (len > 1e-6f) {
            nx /= len;
            ny /= len;
            const float radius = std::sqrt(std::abs(p.radial)) * thick;
            const float ca = std::cos(p.angular);
            const float sa = std::sin(p.angular);
            out[0] += nx * ca * radius;
            out[1] += ny * ca * radius;
            out[2] += sa * radius * 1.35f;
        }
        usedPose = true;
        return;
    }
